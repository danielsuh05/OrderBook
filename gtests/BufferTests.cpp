...
bool waitForCondition(std::function<bool()> condition,
                      std::chrono::milliseconds timeout = maxWait) {
...
class BufferTest : public ::testing::Test {
 protected:
  int pipefd[2];
  void SetUp() override { ASSERT_EQ(pipe(pipefd), 0); }
...
TEST_F(BufferTest, ReadOperationUpdatesLimit) {
  ...
  waitForCondition(
      [&buffer, testDataSize]() { return buffer.limit_ >= testDataSize; });
...
TEST_F(BufferTest, SaveAfterPosShiftsData) {
  ...
  EXPECT_EQ(
      std::strncmp(buffer.ptr_, testData + consume, testDataSize - consume), 0);
}
...
TEST_F(BufferTest, ReadMoreThanBufferCapacityAndSaveAfterPos) {
  ...
  waitForCondition(
      [&buffer, bufferSize]() { return buffer.limit_ >= bufferSize; });
